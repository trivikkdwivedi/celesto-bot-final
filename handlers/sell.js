// handlers/sell.js — Jupiter SELL command (quote → swap → sign → send)
const tokenService = require("../services/token");
const tradeService = require("../services/trade");
const signer = require("../services/signer");
const walletService = require("../services/wallet");

async function sellCommand(ctx) {
  try {
    const parts = ctx.message?.text?.split(/\s+/).slice(1) || [];
    if (parts.length < 3) {
      return ctx.reply(
        "Usage:\n" +
        "`/sell <input_token> <output_token> <amount>`\n\n" +
        "Example:\n`/sell BONK SOL 1000000`",
        { parse_mode: "Markdown" }
      );
    }
    ...
    console.error("sellCommand error:", err);
    return ctx.reply(
